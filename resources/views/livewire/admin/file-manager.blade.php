    ...
    <div class="file-manager-container" 
        ...
        <div class="file-manager-wrapper container-fluid px-3">
                                ...
                                <div class="card h-100 file-card border-0 shadow-sm {{ in_array($file->id, $selectedFiles) ? 'selected-border' : '' }}" style="transition: transform 0.2s;">
                                    ...
                                    <div class="card-body p-2 bg-white border-top">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            <span class="text-muted text-uppercase" style="font-size: 0.55rem;">{{ pathinfo($file->file_path, PATHINFO_EXTENSION) }}</span>
            ...
            <div x-show="showModal" class="modal-backdrop-custom" 
                ...
                <div class="modal-dialog modal-dialog-centered shadow-lg" style="width: 100%; max-width: 500px;">
                    <div class="modal-content border-0" style="border-radius: 15px; background: #fff;">
                        <form wire:submit.prevent="save">
                            <div class="modal-header border-0 bg-light px-4 pt-4 d-flex justify-content-between">
                                <h6 class="modal-title font-weight-bold">{!! $editingFileId ? 'Edit' : 'Upload' !!}</h6>
                                <button type="button" class="close border-0 bg-transparent" @click="showModal = false">&times;</button>
                            </div>
                            <div class="modal-body p-4">
                                @if(!$editingFileId)
                                    <div class="upload-area p-5 text-center border-dashed rounded-lg" 
                                        ...
                                        <div wire:loading.remove wire:target="file">Upload File</div>
                                        <div wire:loading wire:target="file">Uploading...</div>
                                    </div>
                                @endif
                                <div class="form-group mt-4">
                                    <input type="text" wire:model="caption" class="form-control rounded-pill px-4" placeholder="Caption">
                                </div>
                            </div>
